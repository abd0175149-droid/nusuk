    ...
    <AppLayout>
        ...
        <div class="space-y-6">
            ...
            <section v-if="transfers.length" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
                ...
                <tbody><tr v-for="t in transfers" :key="t.id" class="border-t"><td class="px-3 py-2 font-mono">{{ t.transfer_number }}</td><td class="px-3 py-2">{{ t.agent?.name }}</td><td class="px-3 py-2 font-mono" dir="ltr">{{ Number(t.amount_sar).toFixed(2) }} SAR</td><td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-xs" :class="{'bg-yellow-100 text-yellow-700':t.status==='pending','bg-green-100 text-green-700':t.status==='approved','bg-red-100 text-red-700':t.status==='rejected'}">{{ {pending:'معلقة',approved:'معتمدة',rejected:'مرفوضة'}[t.status] }}</span></td></tr></tbody></table>
            </section>
            ...
            <section v-if="receipts.length" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
                ...
                <tbody><tr v-for="r in receipts" :key="r.id" class="border-t"><td class="px-3 py-2 font-mono">{{ r.receipt_number }}</td><td class="px-3 py-2">{{ r.client?.name }}</td><td class="px-3 py-2 font-mono" dir="ltr">{{ Number(r.amount_jod).toFixed(3) }} JOD</td><td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-xs" :class="{'bg-yellow-100 text-yellow-700':r.status==='pending','bg-green-100 text-green-700':r.status==='approved','bg-red-100 text-red-700':r.status==='rejected'}">{{ {pending:'معلقة',approved:'معتمدة',rejected:'مرفوضة'}[r.status] }}</span></td></tr></tbody></table>
            </section>
            ...
            <section v-if="invoices.length" class="bg-white rounded-2xl shadow-sm border border-gray-200 p-5">
                ...
                <tbody><tr v-for="i in invoices" :key="i.id" class="border-t"><td class="px-3 py-2 font-mono">{{ i.invoice_number }}</td><td class="px-3 py-2">{{ i.agent?.name }}</td><td class="px-3 py-2">{{ i.client?.name }}</td><td class="px-3 py-2 font-mono" dir="ltr">{{ Number(i.total_jod).toFixed(3) }}</td><td class="px-3 py-2"><span class="px-1.5 py-0.5 rounded text-xs" :class="{'bg-yellow-100 text-yellow-700':i.status==='pending','bg-green-100 text-green-700':i.status==='approved','bg-red-100 text-red-700':i.status==='rejected'}">{{ {pending:'معلقة',approved:'معتمدة',rejected:'مرفوضة'}[i.status] }}</span></td></tr></tbody></table>
            </section>
