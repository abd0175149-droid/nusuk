    ...
    <AppLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <button @click="openModal(null)" class="px-5 py-2.5 rounded-xl font-bold text-sm text-black bg-gradient-to-r from-gold-500 to-gold-400 shadow-md">+ نوع مخالفة</button>
            </div>
            <div class="rounded-xl border overflow-hidden shadow-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"><table class="w-full text-sm">
                ...
                <tbody>
                    <tr v-for="vt in types.data" :key="vt.id" class="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                        <td class="px-5 py-3 font-medium text-gray-800 dark:text-gray-100">{{ vt.name }}</td>
                        <td class="px-5 py-3 font-mono text-xs text-gold-700">{{ vt.code }}</td>
                        <td class="px-5 py-3 font-mono" dir="ltr">{{ Number(vt.default_cost_sar).toLocaleString('en',{minimumFractionDigits:2}) }}</td>
                        <td class="px-5 py-3"><span class="px-2.5 py-1 rounded-full text-xs font-bold" :class="vt.is_active?'bg-green-100 text-green-700':'bg-red-100 text-red-700'">{{ vt.is_active?'نشط':'معطل' }}</span></td>
                        <td class="px-5 py-3 text-center">
                            <button @click="openModal(vt)" class="px-2 py-1 text-xs text-gold-700 hover:bg-gold-50 rounded-lg">✏️</button>
                            <button @click="del(vt)" class="px-2 py-1 text-xs text-red-600 hover:bg-red-50 rounded-lg">🗑️</button>
                        </td>
                    </tr>
