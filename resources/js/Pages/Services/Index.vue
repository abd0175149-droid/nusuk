    ...
    <AppLayout>
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <button @click="openModal(null)" class="px-5 py-2.5 rounded-xl font-bold text-sm text-black bg-gradient-to-r from-gold-500 to-gold-400 shadow-md">+ خدمة جديدة</button>
            </div>
            <div class="rounded-xl border overflow-hidden shadow-sm bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700"><table class="w-full text-sm">
                <thead><tr class="bg-gray-50 text-gray-600">
                    ...
                    <th class="px-5 py-3 text-right font-bold">السعر (SAR)</th>
                    <th class="px-5 py-3 text-right font-bold">السعر (JOD)</th>
                ...
                <tbody>
                    <tr v-for="s in services.data" :key="s.id" class="border-t border-gray-100 dark:border-gray-700/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                        ...
                        <td class="px-5 py-3 font-mono text-xs text-gold-700">{{ s.code }}</td>
                        <td class="px-5 py-3 font-mono" dir="ltr">{{ Number(s.default_price_sar).toLocaleString('en',{minimumFractionDigits:2}) }}</td>
                        <td class="px-5 py-3 font-mono" dir="ltr">{{ Number(s.default_price_jod).toLocaleString('en',{minimumFractionDigits:3}) }}</td>
                    ...
                    <tr v-if="!services.data?.length"><td colspan="6" class="px-5 py-12 text-center text-gray-400">لا يوجد خدمات</td></tr>
                </tbody>
            </table></div>
        </div>
        <!-- Form Modal -->
            ...
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6">
                ...
                <form @submit.prevent="submit" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">السعر SAR *</label><input v-model="form.default_price_sar" type="number" step="0.01" required dir="ltr" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-gold-500 focus:outline-none"/></div>
                        <div><label class="block text-sm font-medium text-gray-700 mb-1">السعر JOD *</label><input v-model="form.default_price_jod" type="number" step="0.001" required dir="ltr" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-gold-500 focus:outline-none"/></div>
                    </div>
...
let t=null;
const form = useForm({ name:'',default_price_sar:'',default_price_jod:'',description:'',is_active:true });
const openModal=(s)=>{ editItem.value=s; form.name=s?.name||''; form.default_price_sar=s?.default_price_sar||''; form.default_price_jod=s?.default_price_jod||''; form.description=s?.description||''; form.is_active=s?.is_active??true; form.clearErrors(); showForm.value=true; };
const submit=()=>{ const o={onSuccess:()=>{showForm.value=false},preserveScroll:true}; editItem.value?form.put('/services/'+editItem.value.id,o):form.post('/services',o); };
