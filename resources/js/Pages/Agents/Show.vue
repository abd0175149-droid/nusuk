        ...
        <div class="space-y-6">
            ...
            <div class="dash-card p-5">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                    ...
                    <div><span class="text-gray-400 text-xs">الرصيد الحالي</span><p class="font-bold font-mono text-lg" dir="ltr" :class="parseFloat(agent.balance_sar)>=0?'text-green-500':'text-red-500'">{{ Number(agent.balance_sar).toLocaleString('en',{minimumFractionDigits:2}) }} SAR</p></div>
                </div>
            </div>
            ...
            <form @submit.prevent="applyFilter" class="flex flex-wrap items-end gap-4">
                ...
                <a :href="'/agents/'+agent.id+'/print-statement?from='+from+'&to='+to" target="_blank" class="px-5 py-2.5 rounded-xl font-bold text-sm text-gray-600 dark:text-gray-300 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800">🖨️ طباعة</a>
                <a href="/agents" class="px-5 py-2.5 rounded-xl text-sm text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800">← الرجوع</a>
            </form>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="dash-card p-4 text-center"><p class="text-xs text-gray-400">الرصيد الختامي</p><p class="font-bold font-mono text-lg mt-1" dir="ltr" :class="parseFloat(agent.balance_sar)>=0?'text-green-500':'text-red-500'">{{ fmt(agent.balance_sar) }}</p></div>
            </div>
