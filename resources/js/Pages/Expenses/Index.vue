        ...
        <div class="space-y-6">
            ...
            <div class="rounded-xl border overflow-hidden shadow-sm bg-white"><table class="w-full text-sm">
                ...
                <tbody><tr v-for="e in expenses.data" :key="e.id" :data-row-id="e.id"
                    ...
                    <td class="px-5 py-3 text-right text-xs max-w-[200px] truncate">{{ e.description }}</td>
                    <td class="px-5 py-3 text-right font-bold font-mono text-xs text-green-600" dir="ltr">{{ Number(e.amount).toLocaleString('en',{minimumFractionDigits:2}) }} {{ e.currency }}</td>
                    ...
                    <td class="px-5 py-3 text-center whitespace-nowrap">
                        ...
                        <template v-if="e.status==='approved'"><button v-if="can('expenses.edit_approved')" @click="startEdit(e)" class="px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded-lg">✏️ تعديل</button></template>
                        <template v-if="e.status==='editing'"><button @click="openEditForm(e)" class="px-2 py-1 text-xs text-blue-600 hover:bg-blue-50 rounded-lg font-bold">📝 تعديل البيانات</button></template>
                    </td>
