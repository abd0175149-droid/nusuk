        ...
        <div v-if="viewClient" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="viewClient=null">
            <div class="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6">
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div><span class="text-gray-400">الرصيد:</span><p class="font-bold font-mono">{{ Number(viewClient.balance_jod).toLocaleString('en',{minimumFractionDigits: viewClient.currency==='SAR'?2:3}) }} {{ viewClient.currency||'JOD' }}</p></div>
...
const submit = () => {
    ...
    const o = { onSuccess:()=>{showForm.value=false; form.reset(); form.clearErrors(); editing.value=null;}, preserveScroll:true };
